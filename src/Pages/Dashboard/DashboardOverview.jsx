...
const DashboradOveriew = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `https://trade-analysis-server.onrender.com/stockdata/unique-trade-codes/`
      );
      if (!response.ok) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `https://trade-analysis-server.onrender.com/stockdata/chart_data/?trade_code=${tradeCode}`
      );
      if (!response.ok) {
        throw new Error("Failed to fetch stock data");
      }
